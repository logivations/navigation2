...
DriveStraightBehaviorTester::DriveStraightBehaviorTester()
: is_active_(false),
  initial_pose_received_(false)
{
  node_ = rclcpp::Node::make_shared("drive_straight_behavior_test");

  tf_buffer_ = std::make_shared<tf2_ros::Buffer>(node_->get_clock());
  tf_listener_ = std::make_shared<tf2_ros::TransformListener>(*tf_buffer_);

  client_ptr_ = rclcpp_action::create_client<DriveStraight>(
  ...
  subscription_ = node_->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
    ...
    std::bind(&DriveStraightBehaviorTester::amclPoseCallback, this, std::placeholders::_1));
...
void DriveStraightBehaviorTester::activate()
{
  if (is_active_) {
  ...
  RCLCPP_INFO(this->node_->get_logger(), "DriveStraight action server is ready");
  is_active_ = true;
}

void DriveStraightBehaviorTester::deactivate()
{
  if (!is_active_) {
...
bool DriveStraightBehaviorTester::defaultDriveStraightBehaviorTest(
  const DriveStraight::Goal goal_msg,
  const double tolerance)
{
  ...
  rclcpp_action::ClientGoalHandle<DriveStraight>::SharedPtr goal_handle = goal_handle_future.get();
  if (!goal_handle) {
  ...
  rclcpp_action::ClientGoalHandle<DriveStraight>::WrappedResult wrapped_result = result_future.get();
...
void DriveStraightBehaviorTester::sendInitialPose()
{
...
void DriveStraightBehaviorTester::amclPoseCallback(
